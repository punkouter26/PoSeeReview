...
public sealed class LeaderboardServiceTests
{
    ...
    public async Task GetTopComicsAsync_WithValidRegion_ReturnsTop10Entries()
    {
        ...
        var entries = GenerateLeaderboardEntries(15, RegionCode.From(region)); // 15 entries but should return top 10
        _mockRepository.Setup(x => x.GetTopEntriesAsync(RegionCode.From(region), 10))
            .ReturnsAsync(entries.Take(10).ToList());

        var service = new LeaderboardService(_mockRepository.Object);

        // Act
        var result = await service.GetTopComicsAsync(RegionCode.From(region), 10);
    ...
    public async Task GetTopComicsAsync_WithFewerThan10Entries_ReturnsAvailableEntries()
    {
        ...
        var entries = GenerateLeaderboardEntries(3, RegionCode.From(region));
        _mockRepository.Setup(x => x.GetTopEntriesAsync(RegionCode.From(region), 10))
    ...
    public async Task UpsertEntryAsync_WithNewEntry_InsertsEntry()
    {
        ...
        {
            PlaceId = PlaceId.From("ChIJTest123"),
            RestaurantName = "Test Restaurant",
            Address = "123 Test St",
            Region = RegionCode.From("US-CA-SF"),
            StrangenessScore = 95,
    ...
    public async Task UpsertEntryAsync_WithHigherScore_UpdatesExistingEntry()
    {
        ...
        var existingEntry = new LeaderboardEntry
        {
            ...
            StrangenessScore = 75,
            Region = RegionCode.From("US-NY-NYC")
        };
        ...
        _mockRepository.Setup(x => x.GetByPlaceIdAsync(PlaceId.From(placeId), RegionCode.From("US-NY-NYC")))
        ...
        // Assert
        _mockRepository.Verify(x => x.UpsertAsync(It.Is<LeaderboardEntry>(
            e => e.PlaceId == PlaceId.From(placeId) && e.StrangenessScore == 90
        )), Times.Once);
    }
    ...
    public async Task GetTopComicsAsync_WithInvalidRegion_ThrowsArgumentException(string? invalidRegion)
    {
        ...
        // Act & Assert — RegionCode.From normalises blanks to the default partition, so the
        // guard is exercised by constructing the blank value directly.
        await Assert.ThrowsAsync<ArgumentException>(
            () => service.GetTopComicsAsync(new RegionCode(invalidRegion!), 10));
    }
    ...
    private static List<LeaderboardEntry> GenerateLeaderboardEntries(int count, RegionCode region)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                Rank = i + 1,
                PlaceId = PlaceId.From($"ChIJTest{i:D3}"),
    ...
    private class LeaderboardService
    {
        ...
        public async Task<List<LeaderboardEntry>> GetTopComicsAsync(RegionCode region, int limit)
        {
            if (region.IsEmpty)
